...
function SpotifyTokenRequest  (functionName:string,account?:string|null) {
    return new Promise<[boolean,string,string]>(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
        ...
        } catch (err) { reject(1); console.log("Spotify"+functionName+"FileError2: " + err); return; }

        if (SpotifyAccessTokenTmp != null && SpotifyAccessTokenTmp != "") {
            httpsRequestPromise("accounts.spotify.com","/api/token","POST",{"Content-Type":"application/x-www-form-urlencoded","Accept":"application/json"},"grant_type=refresh_token&refresh_token=" + SpotifyAccessTokenTmp + "&client_id=" + SpotifyClientID + "&client_secret=" + SpotifyClientSecret)
            .then ((curl:string) => {
                try {
                    if (curl == "") { reject([2]); }
                    else if (JSON.parse(curl)) {
                        ...
                        if (json.error == null) {
                            ...
                            try {
                                ...
                                reject(3); //console.log("Spotify"+functionName+"TryCatchError2: " + err);
                            }
                        } else {
                            reject(4); console.log("Spotify"+functionName+"ServerError1: " + json.error);
                        }
                    } else {
                        reject(5); console.log("Spotify"+functionName+"JsonError4: could not parse json");
                    }
                } catch (err) {
                    reject(6); console.log("Spotify"+functionName+"TryCatchError3: " + err);
                }
            }).catch((err:Error) => {
                reject(7); console.log("Spotify"+functionName+"HttpsRequestError4: " + err);
            });
        } else {
            reject(8); console.log("Spotify"+functionName+"TokenError3: Spotify refresh token not found");
        }
    });
}
...
function SpotifyMeRequest     (functionName:string, account:string|null, path:string, method:"GET"|"POST"|"PUT", data:string|null, refresh:boolean=false, onrefresh?:(()=>void)):Promise<any> {
    return new Promise<any>(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
        var SpotifyAccessTokenTmp:null|string = null;
        try { SpotifyAccessTokenTmp = fs.readFileSync(__dirname+"/Spotify/AccessToken" + (account||"") + ".txt", 'utf8');
        } catch (err) { reject([1]); console.log("Spotify"+functionName+"FileError1: " + err); return; }//error code 1  or error code 19
        ...
        .then ((curl:string) => {
            try {
                if (curl == "") { reject([2]); }//error code 2  or error code 20
                    ...
                    if (json.error != null) {
                        if (refresh) {
                            if (json.error.message == "The access token expired" || json.error.message == "Invalid access token") {
                                ...
                                reject([13,json.error]); //console.log("Spotify"+functionName+"JsonError1: " + JSON.stringify(json.error));//error code 13 or error code 31
                            }
                        }else {
                            if (json.error.message == "The access token expired") {
                                reject([11]); console.log("Spotify"+functionName+"TokenError1: Spotify refresh token expired.");//error code 11 or error code 29
                            } else if (json.error.message == "Invalid access token") {
                                reject([12]); console.log("Spotify"+functionName+"TokenError2: Access token is invalid, Reauthentication is required.");//error code 12 or error code 30
                            } else {
                                reject([13]); //console.log("Spotify"+functionName+"JsonError2: " + JSON.stringify(json.error));//error code 13 or error code 31
                            }
                        }
                    } else {
                        resolve(json);
                    ...
                    reject([14]); console.log("Spotify"+functionName+"JsonError3: could not parse json");//error code 14 or error code 32
                }
            } catch (err:any) {
                if (err.toString() != "SyntaxError: Unexpected end of JSON input") {
                    reject([15]); console.log("Spotify"+functionName+"TryCatchError1: " + err);//error code 15 or error code 33
                } else {console.log(err);}
            }
        }).catch((err:Error) => {
            if (("" + err).startsWith("Error: getaddrinfo ENOTFOUND "   )) {
                reject([16]); console.log("Spotify"+functionName+"HttpsRequestError1: " + (""+err).slice(29) + " address info not found.");//error code 16 or error code 34
            } else if (("" + err).startsWith("Error: connect ETIMEDOUT ")) {
                reject([17]); console.log("Spotify"+functionName+"HttpsRequestError2: " + (""+err).slice(25) + " connection timed out."  );//error code 17 or error code 35
            } else {
                reject([18]); console.log("Spotify"+functionName+"HttpsRequestError3: " + err);//error code 18 or error code 36
            }
        });
    });
}
...
function getSpotifyDevices  (functionName:string, account:string|null                 ):Promise<string[]> {
    return new Promise<string[]>(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
        ...
        .catch(async(out:[number,any])=>{reject(out[0])});
    });
}
//#endregion
                ...
                if (err.message == "Device not found") {
                    ...
                    .then(async(list:any)=>{
                        if (list.length > 0) {
                            ...
                            .then(resolve)
                            .catch((out:[number,any])=>{reject([out[0]+117,out[1]])});//maps errors 1-78 into 118-156
                        } else { console.log("No device is avaliable to play music."); }
                    })
                    .catch((out:[number,any])=>{reject([out[0]+78,out[1]])});//maps errors 1-39 into 79-117
                    ...
                    SpotifyPlay(null,device,account).then(resolve).catch((out:[number,any])=>{reject([out[0]+39,out[1]])});//maps errors 1-39 into 40-78
                } else if (err.status == 502) {
                    reject(38); console.log("SpotifyPlayHttpsError1: \"Bad gateway.\"");
                } else {
                    reject(39); console.log("SpotifyPlayJsonError4: " + JSON.stringify(err));
                }
            } else if (out[0] == 2) {
                resolve(true);
            } else reject(out[0]);
        ...
        .catch(async(out:[number,any])=>{
            if (out[0]==13||out[0]==31) {
                ...
                if (err.message == "Device not found") {
                    ...
                    reject(39); console.log("SpotifyPauseHttpsError1: \"Bad gateway.\"");
                } else {
                    reject(40); console.log("SpotifyPauseJsonError4: " + JSON.stringify(err));
                }
            } else if (out[0] == 2) {
                resolve(false);
            } else reject(out[0]);
 ...
 */ 
function SpotifyStatus      (                                      account:string|null                     ):Promise<any[]   > {
    return new Promise<any[]  >(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
                ...
                if (err.message == "Device not found") {
                    ...
                    .then(async(list:any)=>{
                        if (list.length > 0) {
                            ...
                            .catch((out:[number,any])=>{reject([out[0]+79,out[1]])});//maps errors 1-79 into 80-158
                        } else { console.log("No device is avaliable to play music."); }
                    })
                    .catch((out:[number,any])=>{reject([out[0]+40,out[1]])});//maps errors 1-39 into 41-79
                } else if (err.message == "Player command failed: Restriction violated") {
                    reject(38); console.log("No clue. ln454 Spotify.js");
                } else if (err.status == 502) {
                    reject(39); console.log("SpotifyPlayHttpsError1: \"Bad gateway.\"");
                } else {
                    reject(40); console.log("SpotifyPlayJsonError4: " + JSON.stringify(err));
                }
            } else if (out[0] == 2) {
                resolve(true);
            } else reject(out[0]);
 ...
 */ 
function SpotifyVolumeUp    (                  device:string|null, account:string|null, amount?:number|null):Promise<void    > {
    return new Promise<void   >(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
        ...
        .then(async(json:any)=>{
            ...
            if (json.device.volume_percent == null) { reject(38); console.log("SpotifyVolumeUpError1: device.volume_percent == null"); return; }
            ...
            .catch((out:[number,any])=>{
                if (out[0] == 2) {
                    resolve(true);
                } else reject([out[0]+38,out[1]]);//maps errors 1-36 into 39-74
            });//maps errors 1-36 into 39-74
        }).catch(reject);
    });
}
 ...
 * code   39-74         are from "/volume" request
 */ 
function SpotifyVolumeDown  (                  device:string|null, account:string|null, amount?:number|null):Promise<void    > {
    return new Promise<void   >(async( resolve:((value?:any)=>void), reject:((reason?:any)=>void) )=>{
        ...
        .then(async(json:any)=>{
            ...
            if (json.device.volume_percent == null) { reject(38); console.log("SpotifyVolumeDownError1: device.volume_percent == null"); return; }
